...
try:
    ...
    ('cannot import utm, make sure utm package is installed')
...
class CarControlPid():
    ...
    def calculate_next_control(self):
        ...
        if self.error > self.dying_zone_acc:
            self.brake = 0.0
            self.throttle = \
...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            origin_lat = 31.23043757444507
            origin_lon = 121.47197969573493
    ...
    def pub_objectlist(self):
        ...
        for index, actor in enumerate(actors):
            ...
            # relative location
            rloc = loc1 - loc0
            loc_vf = Unreal2VF(theta1_rad, rloc.x, rloc.y)
            loc = POSITION()
            loc.x = loc_vf[0]
            loc.y = loc_vf[1]
            ...
            for i in range(0, 5):
                pred = POSITION()
                x_tmp = rloc.x + i * v[0] + i * i * a[0] / 2
                y_tmp = rloc.y + i * v[1] + i * i * a[1] / 2
                x, y = Unreal2VF(theta1_rad, x_tmp, y_tmp)
                pred.x = x
                pred.y = y
...
class KeyboardControl(object):
    ...
    def _parse_xcm_control(self):
        ...
        self.pid_controller.k_p_brake = 0.1
        self.pid_controller.k_i_brake = 0
        self.pid_controller.k_d_brake = 0.2
        self.pid_controller.dying_zone_steer = 0.004 # 1.08deg
        self.pid_controller.dying_zone_acc = 0.01 # 3m/s.delta_t ~ 0.75
        self.pid_controller.dying_zone_brake = 5.5 # -3m/s.delta_t ~ 0.75 delta_t about 0.05s
...
class GnssSensor(object):
    ...
    def pub_navinfo(freq):
        global navinfo, tunnel, xcm_version
        if xcm_version=="LCM":
            try:
                while True:
                    tunnel.publish('NAVINFO', navinfo.encode())
    ...
    def pub_caninfo(freq):
        global caninfo, tunnel, xcm_version
        if xcm_version=="LCM":
            try:
                while True:
                    tunnel.publish('CANINFO', caninfo.encode())
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction, gnss_sensor):
        ...
        self._camera_transforms = [
            (carla.Transform(carla.Location(x=0.0, y=0.0, z=3.0), carla.Rotation(pitch=0.0, roll=0, yaw=0)), Attachment.Rigid),
        ...
        for item in self.sensors:
            ...
            if item[0].startswith('sensor.camera'):
                ...
                bp.set_attribute('rotation_frequency', str(30))#20
                bp.set_attribute('channels', str(64))
                bp.set_attribute('points_per_second', str(250000))#450000
